...
(function () {
  ...
  let roots = [];
  const ROOT_SELECTOR =
    '#obaksa-world-footer.ob-world-footer, section.ob-world-footer[aria-label="세계 시계와 날씨"]';
  ...
  function makeCard(city) {
    return [
      ...
      '<div class="ob-world-city"><span class="ob-world-city-name">' +
        city.label +
        '</span><span class="ob-world-mobile-date" data-mobile-date>----</span><span class="ob-world-tz">' +
        city.tz +
        "</span></div>",
  ...
  function tick() {
    roots.forEach(function (root) {
      CONFIG.cities.forEach(function (city) {
        ...
        if (mobileDateEl)
          mobileDateEl.textContent = dateText(city.tz)
            .replace(/\. /g, ".")
            .replace(/\.$/, "");
        if (hourEl)
  ...
  function applyWeatherToAll(city, data) {
    const code = safeNum(
      data && data.weather && data.weather[0] && data.weather[0].id,
    );
    ...
    roots.forEach(function (root) {
      ...
      if (cond)
        cond.innerHTML =
          '<span class="ob-world-weather-emoji">' +
          emoji +
          '</span><span class="ob-world-cond-label">' +
          codeToLabel(code) +
          "</span>";
      if (compact)
        compact.innerHTML =
          '<span class="ob-world-weather-emoji">' +
          emoji +
          '</span><span class="ob-world-weather-mini">' +
          tempText +
          "℃ · " +
          humText +
          "%</span>";
      ...
      if (windEl)
        windEl.textContent = wind == null ? "--" : String(Math.round(wind));
    });
  }
  ...
  async function fetchWeather(city) {
    const url =
      ...
      "&lon=" +
      encodeURIComponent(city.lon) +
      "&appid=" +
      encodeURIComponent(CONFIG.weatherApiKey) +
  ...
  async function refreshWeather() {
    ...
    try {
      const results = await Promise.all(
        CONFIG.cities.map(function (city) {
      ...
      results.forEach(function (item) {
        applyWeatherToAll(item.city, item.data);
      });
      syncLabel(
        "날씨 업데이트: " +
  ...
  function init() {
    roots = Array.from(document.querySelectorAll(ROOT_SELECTOR)).filter(
      function (root) {
    ...
    if (!window.__obaksaWorldWeatherTimer) {
      window.__obaksaWorldWeatherTimer = setInterval(
        refreshWeather,
